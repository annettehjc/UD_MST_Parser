            ...
            for child, _, _ in self.get_children(node):
                if child not in visited:
                    ...
                    while curr != start:
                        curr = visited[curr]
                        path.append(curr)
                    # Fix 1: (tested with: test_find_cycle_not_exists)
                    if child in path:
                        i = path.index(child)
...
def mst_parse(sent, score_fn, deprels=UDREL):
    ...
    while len(cycle):
        ...
        # Fix 2: remove the unnecessary head (tested with: test_mst_parse)
        mst.remove_edge(oldhead, bestv)
...
def evaluate(gold_sent, pred_sent):
    ...
    for i, gold in enumerate(gold_sent[1:], start=1):
        ...
        if gold.head == pred.head:
            uas += 1
            # Fix 3: LAS must be calculated as a followup of UAS because in UAS we check the head
            # and if it matches we check the labels for LAS (tested with: test_evaluate)
